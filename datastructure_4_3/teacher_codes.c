...
//Error	LNK2019	unresolved external symbol _MergeSort referenced in 
//function _MergeSortWrongVersion	Project11	
//C:\Users\user\Desktop\test\Project11\Project11\Source.obj	1	
// KEYWORD unresolved external symbol

/*how to increase stack size?
Project -> "project name" properties -> linker -> system -> stack commit (reserve) size
*/
...
* Use a for loop to decide the value to result[writePos], writePos = 0 ... arrSize1 + arrSize2-1
...
3. Merge arr[0...n/2-1] and arr[n/2...n-1] and store the result to int result[]
4. Copy the contents of result to arr
*/
// this implemtation is wrong as the function nerver ends, causing stack overflow
void MergeSortWrongVersion(int arr[], int n) {
	MergeSortWrongVersion(arr, n / 2); // key ideas 1 and 2
	MergeSortWrongVersion(arr + n / 2, n - n / 2); // key ideas 1 and 2
...
void MergeSort(int arr[], int n) {
	if (n <= 1) {
		/* instead of n == 1, n <= 1 is safer and captures more terminating conditions
		for example, what if the array originally has size 2 and then becomes size 0
		*/
		return;
	}
